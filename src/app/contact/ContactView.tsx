...
export type ContactSocials = { github: string; linkedin: string; email: string };
...
export default function ContactView({ socials: SOCIALS }: { socials: ContactSocials }) {
      ...
      <section className="relative pb-20 sm:pb-24">
        <div className="mx-auto max-w-6xl px-5 sm:px-8">
          <div className="grid gap-6 lg:grid-cols-[1fr_1fr]">
            <div>
                  ...
                  <Reveal key={o.title} delay={Math.min(i * 60, 200)}>
                    <button
                      ...
                      className={`w-full rounded-2xl border p-4 text-left transition-all duration-300 ${
                        form.topic === o.title
                          ? "border-cyan-400/40 bg-cyan-400/[0.07] shadow-[0_10px_30px_-10px_rgba(34,211,238,0.4)] light:border-cyan-700/40 light:bg-cyan-700/[0.07] light:shadow-[0_10px_30px_-10px_rgba(2,132,199,0.3)]"
                          : "glass hover:border-white/20 light:hover:border-slate-900/20"
                      }`}
                    >
              ...
              <Reveal delay={120}>
                <div className="glass mt-5 rounded-2xl p-5">
                  <p className="flex items-center gap-2 text-[13px] font-medium text-slate-300 light:text-slate-600">
                    <LocationIcon className="h-4 w-4 text-cyan-400" />
                    Based in Bangladesh • working worldwide, remotely
                  </p>
                  <div className="mt-4 flex flex-wrap gap-2">
                    <a href={SOCIALS.github} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.05] px-5 py-2.5 text-[13px] font-semibold text-white transition-all hover:border-white/25 hover:bg-white/10 light:border-slate-900/15 light:bg-white light:text-slate-800 light:shadow-sm light:hover:border-slate-900/30">
                      <GithubIcon className="h-4 w-4" /> GitHub
                    </a>
                    <a href={SOCIALS.linkedin} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.05] px-5 py-2.5 text-[13px] font-semibold text-white transition-all hover:border-white/25 hover:bg-white/10 light:border-slate-900/15 light:bg-white light:text-slate-800 light:shadow-sm light:hover:border-slate-900/30">
            ...
            <Reveal delay={100}>
              <div className="rounded-3xl border border-white/10 bg-black/30 p-6 backdrop-blur-md sm:p-8 light:border-slate-900/10 light:bg-white/80 light:shadow-[0_20px_60px_-25px_rgba(15,23,42,0.2)]">
                <form onSubmit={handleSubmit} className="grid gap-4">
                  ...
                  <div>
                    <label htmlFor="topic" className="mb-1.5 block text-[12.5px] font-semibold text-slate-300 light:text-slate-700">Topic</label>
                    <select id="topic" value={form.topic} onChange={(e) => setForm({ ...form, topic: e.target.value })}
                      className="w-full rounded-xl border border-white/10 bg-white/[0.05] px-4 py-3 text-[14px] text-white outline-none transition-all focus:border-cyan-400/50 light:border-slate-900/15 light:bg-white light:text-slate-900 light:focus:border-cyan-700/50 [&>option]:bg-[#0a0f1e] light:[&>option]:bg-white">
                      {OPTIONS.map((o) => (
