            ...
            <div className="glass flex flex-col gap-3 rounded-3xl p-4 sm:flex-row sm:items-center">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full rounded-2xl border border-white/10 bg-white/[0.04] py-3.5 pl-11 pr-4 text-[14px] text-white placeholder:text-slate-600 outline-none transition-all focus:border-cyan-400/50 focus:ring-2 focus:ring-cyan-400/20 light:border-slate-900/15 light:bg-white light:text-slate-900 light:placeholder:text-slate-400 light:focus:border-cyan-700/50 light:focus:ring-cyan-700/20"
                />
              </div>
                  ...
                  <button
                    ...
                    className={`rounded-full px-4 py-2 text-[12px] font-semibold transition-all ${
                      area === a
                        ...
                        : "border border-white/10 bg-white/[0.03] text-slate-400 hover:text-white light:border-slate-900/10 light:bg-white light:text-slate-500 light:hover:text-slate-900"
                    }`}
                  >
                    {a}
                  </button>
            ...
            {visible.map((n, i) => (
              <Reveal key={n.title} delay={Math.min((i % 4) * 70, 200)}>
                <article className="glass card-hover h-full rounded-3xl p-6">
                  ...
                  <p className="mt-2 text-[13px] leading-relaxed text-slate-400 light:text-slate-600">{n.summary}</p>
                  <div className="mt-4 flex flex-wrap gap-1.5">
                    {n.tags.map((t) => (
                      <span key={t} className="rounded-md bg-white/[0.05] px-2.5 py-1 font-mono text-[11px] text-slate-400 light:bg-slate-900/[0.05] light:text-slate-500">
                        #{t}
                      </span>
                    ))}
                  </div>
