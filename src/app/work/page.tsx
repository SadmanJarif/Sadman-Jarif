...
import { FREELANCE_INFO } from "@/data/site";
import { getExperience, seoMeta } from "@/lib/cms";
            ...
            <Reveal key={r.org} delay={Math.min(i * 80, 160)}>
              <article className="glass relative overflow-hidden rounded-3xl p-7 sm:p-10">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  {r.current && (
                    <span className="rounded-full border border-emerald-400/25 bg-emerald-400/10 px-3 py-1 text-[10.5px] font-bold uppercase tracking-widest text-emerald-200 light:border-emerald-700/25 light:bg-emerald-700/[0.08] light:text-emerald-800">
                      Current
                    </span>
                  )}
                ...
                <div className="mt-7 grid gap-6 lg:grid-cols-2">
                  ...
                  <div className="grid gap-4">
                    <div className="rounded-2xl border border-white/[0.07] bg-white/[0.03] p-5 light:border-slate-900/10 light:bg-slate-900/[0.02]">
                      <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-violet-300 light:text-violet-800">What I learned</p>
                      <p className="mt-2 text-[13.5px] leading-relaxed text-slate-300 light:text-slate-700">{r.learned}</p>
                    </div>
                    <div className="rounded-2xl border border-white/[0.07] bg-white/[0.03] p-5 light:border-slate-900/10 light:bg-slate-900/[0.02]">
                      <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-emerald-300 light:text-emerald-800">Impact / outcome</p>
                      <p className="mt-2 text-[13.5px] leading-relaxed text-slate-300 light:text-slate-700">{r.impact}</p>
                    </div>
                  </div>
                </div>
              </article>
        ...
        <div className="mx-auto max-w-6xl px-5 sm:px-8">
          ...
          <Reveal delay={100}>
            <div className="mx-auto mt-10 max-w-3xl rounded-3xl border border-white/[0.08] bg-gradient-to-br from-white/[0.05] to-white/[0.01] p-7 text-center sm:p-10 light:border-slate-900/10 light:from-white light:to-slate-50 light:shadow-[0_18px_50px_-24px_rgba(15,23,42,0.18)]">
              ...
              <Link href="/contact" className="group mt-7 inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/[0.05] px-6 py-3 text-[14px] font-semibold text-white transition-all hover:border-white/25 hover:bg-white/10 light:border-slate-900/15 light:bg-white light:text-slate-800 light:shadow-sm light:hover:border-slate-900/30">
