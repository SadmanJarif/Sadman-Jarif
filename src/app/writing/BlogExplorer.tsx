...
export default function BlogExplorer({ posts }: { posts: BlogPost[] }) {
  ...
  if (!featured) {
    return (
      <main className="relative min-h-screen bg-[#04060d] px-5 py-32 text-center text-[14px] text-slate-400 light:bg-[#f3f5fa] light:text-slate-500">
                ...
                <button
                  ...
                  className={`rounded-full px-4 py-2 text-[12.5px] font-semibold transition-all duration-300 light:shadow-sm ${
                    filter === c
                      ...
                      : "border border-white/10 bg-white/[0.04] text-slate-400 hover:border-white/20 hover:text-white light:border-slate-900/10 light:bg-white light:text-slate-600 light:hover:border-slate-900/25 light:hover:text-slate-900"
                  }`}
                >
                  {c}
                </button>
            ...
            {(filter === "All" ? rest : visible).map((p, i) => (
              <Reveal key={p.slug} delay={(i % 3) * 80} className="h-full">
                <Link href={`/writing/${p.slug}`} className="glass card-hover group flex h-full flex-col rounded-3xl p-6 sm:p-7">
                  ...
                  <p className="mt-2.5 flex-1 text-[13px] leading-relaxed text-slate-400 light:text-slate-600">{p.excerpt}</p>
          ...
          <div className="mt-12 grid gap-5 lg:grid-cols-2">
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  {topics.map((t) => (
                    <button
                      key={t}
                      onClick={() => setFilter(t)}
                      className="rounded-full border border-white/[0.08] bg-white/[0.03] px-4 py-1.5 text-[12px] font-medium text-slate-400 transition-colors hover:border-cyan-400/30 hover:text-white light:border-slate-900/10 light:bg-white light:text-slate-500 light:shadow-sm light:hover:border-cyan-700/40 light:hover:text-slate-900"
                    >
                      {t}
                    </button>
                  ))}
            ...
            <Reveal delay={100}>
              <div className="glass h-full rounded-3xl p-7">
                ...
                <p className="mt-2 text-[13.5px] leading-relaxed text-slate-400 light:text-slate-600">
