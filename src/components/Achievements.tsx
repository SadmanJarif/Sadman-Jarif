          ...
          {ITEMS.map((a, i) => (
            <Reveal key={a.title} delay={(i % 4) * 80}>
              <div className="glass card-hover group h-full rounded-2xl p-5">
                <span className="inline-flex h-8 w-8 items-center justify-center rounded-lg bg-gradient-to-br from-emerald-400/25 to-cyan-500/25 text-emerald-300">
                  <CheckIcon className="h-4 w-4" />
                </span>
                <p className="mt-3 text-[11px] font-bold uppercase tracking-[0.14em] text-slate-500">
                  {a.org}
                </p>
                <h3 className="font-display mt-1 text-[15px] font-bold leading-snug text-white">
                  {a.title}
                </h3>
