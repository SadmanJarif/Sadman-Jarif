...
export default function Contact({ content }: { content?: Partial<ContactContent> }) {
  const headline = content?.headline || "Let’s build something meaningful.";
  const text =
    content?.text ||
    "Open to connecting for collaboration, projects, AI and software opportunities, entrepreneurship, research and professional networking. I reply fast.";
  const email = content?.email || SOCIALS.email;
  const github = content?.github || SOCIALS.github;
  const linkedin = content?.linkedin || SOCIALS.linkedin;
  const emailUser = email.replace(/^mailto:/, "");
  ...
  return (
    <section id="contact" className="relative scroll-mt-28 py-20 sm:py-28">
      <div className="mx-auto max-w-6xl px-5 sm:px-8">
        <Reveal>
          <div className="relative overflow-hidden rounded-[2rem] border border-white/[0.09] bg-gradient-to-br from-[#0a0f22] via-[#0c0a26] to-[#140b24] p-8 sm:p-12 lg:p-14 light:border-slate-900/10 light:from-white light:via-indigo-50/50 light:to-fuchsia-50/50 light:shadow-[0_30px_80px_-30px_rgba(99,102,241,0.3)]">
            ...
            <div className="relative grid gap-10 lg:grid-cols-[1fr_0.95fr] lg:gap-14">
              <div>
                ...
                <h2 className="font-display mt-5 text-3xl font-extrabold leading-[1.08] tracking-tight text-white sm:text-[2.75rem] light:text-slate-900">
                  {headline}
                </h2>
                <p className="mt-4 max-w-md text-[14.5px] leading-relaxed text-slate-300/90 light:text-slate-600">
                  {text}
                </p>
                ...
                <div className="mt-5 flex flex-wrap gap-2">
                  {REASONS.map((r) => (
                    <span
                      key={r}
                      className="rounded-full border border-white/10 bg-white/[0.05] px-3.5 py-1.5 text-[12px] font-medium text-slate-200 light:border-slate-900/10 light:bg-white light:text-slate-600 light:shadow-sm"
                    >
                      {r}
                    </span>
                  ))}
                ...
                <div className="mt-7 flex flex-wrap gap-3">
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="inline-flex items-center gap-2 rounded-full border border-white/12 bg-white/[0.05] px-5 py-2.5 text-[13.5px] font-semibold text-white transition-all hover:-translate-y-0.5 hover:border-white/25 hover:bg-white/10 light:border-slate-900/15 light:bg-white light:text-slate-800 light:shadow-sm light:hover:border-slate-900/30"
                  >
              ...
              <div className="rounded-3xl border border-white/10 bg-black/30 p-6 backdrop-blur-md sm:p-7 light:border-slate-900/10 light:bg-white/80 light:shadow-[0_20px_60px_-25px_rgba(15,23,42,0.2)]">
