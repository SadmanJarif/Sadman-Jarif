          ...
          <div className="grid gap-5">
            {ROLES.map((r, i) => (
              <Reveal key={r.org} delay={i * 100}>
                <article className="glass card-hover relative ml-0 rounded-3xl p-6 pl-14 sm:p-7 sm:pl-16">
                  ...
                  >
                    <span className={`absolute h-full w-full rounded-full bg-gradient-to-br ${r.accent} opacity-90`} />
                  ...
                  <h3 className="font-display mt-2.5 text-lg font-bold leading-snug text-white sm:text-xl">
                    {r.role} <span className="text-slate-500">—</span>{" "}
                    <span className="text-gradient-cyan">{r.org}</span>
                  </h3>
                  <p className="mt-2.5 text-[14px] leading-relaxed text-slate-400">{r.text}</p>
                  <div className="mt-4 flex flex-wrap gap-2">
                    {r.points.map((p) => (
                      <span
                        key={p}
                        className="rounded-full border border-white/10 bg-white/[0.04] px-3 py-1.5 text-[12px] font-medium text-slate-300"
                      >
                        {p}
                      </span>
                    ))}
            ...
            <Reveal delay={100}>
              <div className="ml-0 rounded-3xl border border-dashed border-white/15 bg-white/[0.015] p-6 pl-14 sm:pl-16">
                <p className="font-display text-[15px] font-bold text-slate-300">
