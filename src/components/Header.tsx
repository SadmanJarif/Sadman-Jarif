  ...
  return (
    <>
      <header className="fixed inset-x-0 top-3 z-50 px-3 sm:top-5 sm:px-6">
        <nav
          className={`mx-auto flex max-w-6xl items-center justify-between gap-3 rounded-2xl px-4 py-3 transition-all duration-500 sm:rounded-full sm:px-5 ${
            scrolled
              ? "glass-strong shadow-[0_18px_50px_-12px_rgba(0,0,0,0.7),0_0_0_1px_rgba(255,255,255,0.06)]"
              : "border border-white/10 bg-[#0a0f1e]/55 shadow-[0_10px_40px_-12px_rgba(0,0,0,0.6)] backdrop-blur-xl"
          }`}
        >
          <Link href="/" className="group flex items-center gap-3">
            <span className="relative flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-cyan-400 via-indigo-500 to-fuchsia-500 text-[15px] font-extrabold text-white shadow-[0_8px_24px_-6px_rgba(99,102,241,0.7)]">
              SJ
              <span className="absolute inset-0 rounded-xl ring-1 ring-inset ring-white/25" />
            </span>
            <span className="leading-tight">
              <span className="font-display block text-[13.5px] font-bold tracking-tight text-white">
                Sadman M. Jarif
              </span>
              ...
              >
                <div className="glass-strong rounded-2xl p-2 shadow-2xl">
                  {MORE_LINKS.map((link) => (
                    ...
                    >
                      <span className={`block text-[13.5px] font-semibold ${isActive(link.href) ? "text-white" : "text-slate-200"}`}>
                        {link.label}
                      </span>
          ...
          <div className="flex items-center gap-2">
            <span className="mr-1 hidden items-center gap-2 rounded-full border border-emerald-400/20 bg-emerald-400/[0.07] px-3 py-1.5 text-[11.5px] font-medium text-emerald-200 md:inline-flex">
