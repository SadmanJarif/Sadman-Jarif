...
/* "Silk" flow-shader background — plain WebGL1, no libraries.
 * Renders the exact 21st.dev Shader Builder fragment shader below on a
 * fullscreen triangle. Used as the hero backdrop (dark theme).
 * Cursor is off (presence 0). DPR capped at 2. RAF pauses when the tab is
 * hidden or the hero scrolls out of view. Renders a single static frame when
 * the user prefers reduced motion. Hides itself if WebGL is unavailable.
 */

const VERT = `attribute vec2 a_position;
void main() { gl_Position = vec4(a_position, 0.0, 1.0); }`;

const FRAG = `// "Silk" — made with the 21st.dev Shader Builder
// Packed WebGL1 uniforms (the shader exposes readable u_* aliases as macros):
//   u_colors[8] (first 5 used)
//   vec3(0.043, 0.067, 0.157)
//   vec3(0.020, 0.282, 1.000)
//   vec3(0.020, 0.282, 1.000)
//   vec3(0.020, 0.282, 1.000)
//   vec3(0.043, 0.067, 0.157)
//   u_scene = vec4(canvas width, canvas height, seconds * 0.76, 5.0)
...
// Palette low → high: #0B1128, #0548FF ×3, #0B1128
...
export default function SilkBackground({ className = "" }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    const u = (name: string) => gl.getUniformLocation(program as WebGLProgram, name);
    ...
    const draw = (elapsed: number) => {
      resize();
      // u_scene = vec4(width, height, seconds * 0.76, 5.0)
      gl.uniform4f(uScene, canvas.width, canvas.height, elapsed * 0.76, 5.0);
  ...
  return (
    <div ref={wrapRef} className={`absolute inset-0 overflow-hidden ${className}`} aria-hidden>
