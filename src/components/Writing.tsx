          ...
          {WRITING.map((w, i) => (
            <Reveal key={w.title} delay={i * 100} className="h-full">
              ...
              <article className="glass card-hover group flex h-full cursor-pointer flex-col rounded-3xl p-6 sm:p-7">
                ...
                <p className="mt-2.5 flex-1 text-[13.5px] leading-relaxed text-slate-400 light:text-slate-600">{w.excerpt}</p>
                <div className="mt-5 flex items-center justify-between border-t border-white/[0.07] pt-4 text-[12.5px]">
                  ...
                  <span className="inline-flex items-center gap-1.5 font-semibold text-cyan-300 transition-all group-hover:gap-2.5 light:text-cyan-700">
        ...
        <Reveal delay={150}>
          <div className="mt-8 flex flex-wrap justify-center gap-2">
            {["AI articles", "Dev lessons", "Research", "Career", "Startups", "EdTech", "Opinions"].map((t) => (
              <span
                key={t}
                className="rounded-full border border-white/[0.08] bg-white/[0.03] px-4 py-1.5 text-[12px] font-medium text-slate-400 light:border-slate-900/10 light:bg-white light:text-slate-500 light:shadow-sm"
              >
                {t}
              </span>
            ))}
